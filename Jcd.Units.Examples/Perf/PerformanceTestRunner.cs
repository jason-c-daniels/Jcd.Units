...
public class PerformanceTestRunner
{
   ...
   public void RunAllTests(bool report = true)
   {
      if (report)
      {
         ...
         Console.WriteLine($"  L2 Cache Size: {cpu.L2CacheSize.As(StorageUnits.Kilobyte).To(StorageUnits.Mebibyte):n2}");

         Console.WriteLine($"  L3 Cache Size: {cpu.L3CacheSize.As(StorageUnits.Kilobyte).To(StorageUnits.Mebibyte):n2}");
      ...
      RunTestsInScenario(report, baselineScenario, BaselinePerfTests.GetAll().ToList());

      var quantityScenarios = Scenarios.GetAll().ToList();

      var quantityTests = PerfTests.GetAll().ToList();

      foreach (var scenario in quantityScenarios)
      {
         RunTestsInScenario(report, scenario, quantityTests);
      }
   }
   ...
   public void RunTest(IPerfTest? test, bool report)
   {
      ...
      if (report)
      {
         Console.WriteLine($"      - Name: {test.Description}");
      }
      ...
      sw.Stop();
      var stats = report
                     ? CalculateStats(sw, operationCount)
                     : RunStats.Empty;
      test.Teardown();
