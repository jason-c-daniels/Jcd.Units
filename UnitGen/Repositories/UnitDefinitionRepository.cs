...
public class UnitDefinitionRepository : IReadOnlyRepository<UnitDefinition>
{
   ...
   private IImmutableDictionary<string, UnitType> _unitTypesByName = new Dictionary<string, UnitType>().ToImmutableDictionary();
   ...
   public IReadOnlyList<UnitDefinition> GetAll()
   {
      ...
      var unitsWithPrefixes =
         ...
         where unit.UsesPrefixes
            && (unit.PrefixExponentsToInclude.ToLower() == "all"         // all explicitly listed
             ...
             || string.IsNullOrWhiteSpace(unit.PrefixExponentsToInclude) // no value is the same as all or any
             || prefix.IsBasePrefix
      ...
      var definedUnits = unitsWithPrefixes.Concat(unitsWithoutPrefixes).ToImmutableList();

      var generatedUnits = GenerateUnits(definedUnits);

      return _allItems = definedUnits.Concat(generatedUnits)
                                     .Distinct()
   ...
   {
      return (from unitDef in GetAll() select unitDef.UnitType).Distinct().ToList();
   }

   public IEnumerable<IGrouping<string, IGrouping<string, UnitDefinition>>> GetSystemToUnitTypeToUnitDefinitionGroupings()
   {
      return
         from unit in GetAll()
         group unit by unit.System.Name
         into systemGroup
         from unitType in (from unit in systemGroup group unit by unit.UnitType.Name)
   ...
   {
      var lengths = definedUnits.Where(u => u.UnitType.Name == "Length").ToImmutableList();

      var durations = definedUnits.Where(u => u.UnitType.Name == "Duration").ToImmutableList();

      var areas = GenerateAreas(lengths).ToImmutableList();

      var volumes = GenerateVolumes(lengths).ToImmutableList();

      var masses = definedUnits.Where(x => x.UnitType.Name == "Mass").ToImmutableList();

      var velocities = GenerateVelocities(lengths, durations).ToImmutableList();

      var accelerations = GenerateAccelerations(lengths, durations).ToImmutableList();

      var densities = GenerateDensities(masses, volumes);

      return areas.Concat(volumes).Concat(densities).Concat(velocities).Concat(accelerations).ToImmutableList();
   }

   private IReadOnlyList<UnitDefinition> GenerateVelocities(ImmutableList<UnitDefinition> lengths, ImmutableList<UnitDefinition> durations)
   {
                          ...
                          select length with
                                 {
                                    Prefix = _noPrefix
                                  , UnitType = velocity
                                  , Unit = new Unit(length.System.Name
                                                  , velocity.Name
                                                  ...
                                                  , "0"
                                                  , (length.Unit.SortIndex + 1) * (length.Prefix.SortIndex + 1) * (duration.Unit.SortIndex + 1)
   ...
   private IReadOnlyList<UnitDefinition> GenerateAccelerations(ImmutableList<UnitDefinition> lengths, ImmutableList<UnitDefinition> durations)
   {
   ...
   private IReadOnlyList<UnitDefinition> GenerateDensities(IReadOnlyList<UnitDefinition> masses, IReadOnlyList<UnitDefinition> volumes)
   {
                             ...
                             select volume with
                                    {
                                       Prefix = _noPrefix
                                     , UnitType = density
                                     , Unit = new Unit(volume.System.Name
                                                     , density.Name
                                                     ...
                                                     , $"({mass.Coefficient})/({volume.Coefficient})"
                                                       ...
                                                       // TODO: Find a way to convert this.
                                                     , "0"
                                                     , (mass.Unit.SortIndex + 1) * (mass.Prefix.SortIndex + 1) * (volume.Unit.SortIndex + 1)
                            ...
                            select volume with
                                   {
                                    ...
                                    , Unit = new Unit(volume.System.Name
                                                    ...
                                                    , $"({mass.Coefficient})/({volume.Coefficient})"
                                                      ...
                                                      // TODO: Find a way to convert this.
                                                    , "0"
                                                    , (mass.Unit.SortIndex + 1) * (mass.Prefix.SortIndex + 1) * (volume.Unit.SortIndex + 1) + volume.Prefix.SortIndex
                                                     )
                                   }).ToImmutableList();

      // ReSharper disable once UnusedVariable
      var baseDensities =
         from baseDensity in allDensities join derivedDensity in allDensities on baseDensity.UnitName equals derivedDensity.BaseUnitName select baseDensity;
