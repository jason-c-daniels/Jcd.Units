...
public record struct LuminousFlux(string Name, string Symbol, double Coefficient=1, double Offset=0)
    : IUnitOfMeasure<LuminousFlux>
{
    ...
    /// <returns>-1 if less than; 1 if greater than; 0 if equals.</returns>
    /// <exception cref="ArgumentException">When the passed in object is not a <c>LuminousFlux</c></exception>
