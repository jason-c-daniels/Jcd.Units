...
Performs a relative comparison between this `Temperature` and another one.
...
The `Temperature` to compare against.
...
When the passed in object is not a `Temperature`