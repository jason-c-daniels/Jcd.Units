...
into the library's standard formula. This is a matter of calculating an `Offset` and `Coefficient`.

The intended audience for this document are software engineers needing to accomodate a new unit conversion. _The reader
must be familiar with basic arithmetic and algebraic concepts._
...
selected to yield an equivalent function. The following sections provide guidance for how to select an appropriate `a`
and `c` for some well known conversion formulas.
...
- When the coefficient in the original formula will be altered to use with the standard formula it's denoted as `a₀`, in
  the original formula.
- When the offset in the original formula will be unaltered, it's denoted as just `c`, in the original formula.
- When the offset in the original formula will be altered to use with the standard formula it's denoted as `c₀`, in the
  original formula.
...
4. Define `a` to be `-a₀` and we have the exact same formula as in _Coefficient With Offset Applied After
   Multiplication_.
...
4. The definition of _derivation_ in use is the first definition given by the Wolfram website:<sup>5</sup> _"A
   ...
   this case `a` and `c` from `a₀` and `c₀` respectively.
